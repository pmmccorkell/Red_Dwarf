...
//double min_thruster_bias=0.028;  //in us, deadzone around 1.5ms where ESC will not operate      // Use this for Blue Robotics ESCs
double min_thruster_bias=0.0001;   //in us, deadzone around 1.5ms where ESC will not operate      // Updated for VESC, 100ns chosen from observed Oscope accuracy of LPC1768' PWM generator.
...
volatile float heading_Kp = 0.008333f;
...
// Set max/min PWM limits for thrusters.
int speed_limit=500;
...
int main() {
    ...
    // Set max PWM to the speed_limit global.
    fwd_port.set_max(speed_limit);
    fwd_star.set_max(speed_limit);
    aft_port.set_max(speed_limit);
    aft_star.set_max(speed_limit);
