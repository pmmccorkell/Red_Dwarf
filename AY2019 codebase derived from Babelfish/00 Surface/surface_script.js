...
function get_offset() {
      var o=(parseInt(document.getElementById("set_offset").value)+0);
      if (o!=prev_offset) {
            send_offset(o);
      }
}
function get_facing() {
      var f=(parseInt(document.getElementById("set_facing").value)+0);
      if (f!=prev_facing) {
            send_facing(f);
      }
}
...
function send_offset(o) {
      var offset, prefix;
      prefix = "off:";
      if (((o>=0) && (o<=360)) || (o==999) || ((o>=851) && (o<=857))) {
            offset=o.toString();
            if (o<10) prefix="off:00";
            else if (o<100) prefix="off:0";
            else prefix="off:";
            send_cmd2(prefix+offset);
            prev_offset=o;
      }
}
function send_facing(f) {
      var facing, prefix;
      prefix = "hea:";
      if (((f>=0) && (f<=360)) || (f==999) || ((f>=831) && (f<=837))) {
            facing=f.toString();
            if (f<10) prefix="hea:00";
            else if (f<100) prefix="hea:0";
            else prefix="hea:";
            send_cmd2(prefix+facing);
            prev_facing=f;
      }
}
...
function start_python() {
  ajax_cmd.open("GET","runpython_surface.php");
  ajax_cmd.send();
}
function stop_python() {
  ajax_cmd.open("GET","stoppython_surface.php");
  ajax_cmd.send();
}
function start_xbox() {
  ajax_cmd.open("GET","runxbox.php");
  ajax_cmd.send();
}
function stop_xbox() {
  ajax_cmd.open("GET","stopxbox.php");
  ajax_cmd.send();
}
function reset_mbed() {
...
setInterval(function updateDataStream() {
    ajax_serial.open("GET","surface_JSON",true);
    ajax_serial.send();
}, 200);
ajax_serial.onreadystatechange = function() {
  if(ajax_serial.readyState == 4 && ajax_serial.status == 200) {
    ...
    document.getElementById("ser_f").value=status["ser_f"];
    document.getElementById("ser_r").value=status["ser_r"];
    document.getElementById("ser_p").value=status["ser_p"];
    document.getElementById("ser_pf").value=status["ser_pf"];
    document.getElementById("ser_sf").value=status["ser_sf"];
    document.getElementById("ser_pa").value=status["ser_pa"];
    document.getElementById("ser_sa").value=status["ser_sa"];
    document.getElementById("ser_sa").value=status["ser_sa"];
    document.getElementById("ser_vel").value=status["ser_vel"];
    document.getElementById("ser_cal").value=status["ser_cal"];
    document.getElementById("ser_status").value=status["ser_status"];
    document.getElementById("fP_gain").value=status["fP_gain"];
    document.getElementById("fI_gain").value=status["fI_gain"];
    document.getElementById("fD_gain").value=status["fD_gain"];
